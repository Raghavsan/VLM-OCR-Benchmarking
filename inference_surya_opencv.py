...
def enhance_image_for_ocr(img_path):
    ...
    # 2. Convert to pure Grayscale
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    
    # 3. Apply CLAHE (Contrast Limited Adaptive Histogram Equalization)
    # This aggressively darkens text and lightens the background
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
    contrast_img = clahe.apply(gray)
    
    # 4. Apply Non-Local Means Denoising
    # This removes digital grain/blur without destroying the sharp edges of letters
    denoised_img = cv2.fastNlMeansDenoising(contrast_img, h=10)
    
    # Convert back to a PIL Image so Surya can read it
